    ...
    public class DashboardController : ApiController
    {
        ...
        public IEnumerable<Models.ViewModels.DashboardSummaryViewModel> DashboardSummary()
        {
                ...
                .Select(s => new TalonAdmin.Models.ViewModels.DashboardSummaryViewModel
            {
                ...
                VouchersUsed = ctx.Vouchers.FilterCountry(this).FilterOrganization(this).Where(d => d.DistributionId == s).Where(v => v.TransactionRecords.Any()  && v.TransactionRecords.Where(t=>t.Status == 2).Any()).Count(),
                ...
                TotalAmount = ctx.Distributions.FilterCountry(this).FilterOrganization(this).Where(d => d.Id == s).Select(d => d.Categories.Select(c => c.Value.HasValue ? (c.NumberOfVouchers * c.Value) : 0).Sum()).Sum() ?? 0,
                IssuedAmount = ctx.Distributions.FilterCountry(this).FilterOrganization(this).Where(d => d.Id == s).Select(d => d.Categories.Select(c => c.Value.HasValue ? (c.IssuedVouchers * c.Value) : 0).Sum()).Sum() ?? 0,
                ClaimedAmount = ctx.VoucherTransactionRecords.FilterCountry(this).FilterOrganization(this).Where(d => d.Voucher.DistributionId == s).Where(c => c.Status == 2).Select(d => d.Voucher.Category.Value.HasValue ? d.Voucher.Category.Value : 0m).Sum() ?? 0,
            });
